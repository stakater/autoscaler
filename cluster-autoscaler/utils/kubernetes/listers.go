...
func NewListerRegistryWithDefaultListers(kubeClient client.Interface, stopChannel <-chan struct{}, hybridCloud bool) ListerRegistry {
    ...
    var readyNodeLister, allNodeLister NodeLister
    if hybridCloud {
		readyNodeLister = NewCloudNodeReadyLister(kubeClient, stopChannel)
		allNodeLister = NewCloudNodeAllLister(kubeClient, stopChannel)
	} else {
...
// NewCloudNodeReadyLister builds a node lister that returns only cloud ready nodes.
func NewCloudNodeReadyLister(kubeClient client.Interface, stopChannel <-chan struct{}) NodeLister {
	return NewNodeLister(kubeClient, IsCloudNodeReadyAndSchedulable, stopChannel)
}
...
// NewCloudNodeAllLister builds a node lister that returns all cloud nodes (ready and unready)
func NewCloudNodeAllLister(kubeClient client.Interface, stopChannel <-chan struct{}) NodeLister {
	return NewNodeLister(kubeClient, IsCloudNode, stopChannel)
}
